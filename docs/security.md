...
LLM provider API keys are read from the environment and must not be stored in `agent.ts` or `beater.toml`. The provider-agnostic deployment surface is `BEATER_LLM_PROVIDER`, `BEATER_LLM_MODEL`, `BEATER_LLM_API_KEY`, and `BEATER_LLM_BASE_URL`; set provider and model together so a key is never sent to the wrong adapter or model family. The runtime refuses `BEATER_LLM_API_KEY` or `BEATER_LLM_BASE_URL` unless `BEATER_LLM_PROVIDER` is explicit, and it requires `BEATER_LLM_MODEL` whenever `BEATER_LLM_PROVIDER` is set so deployment overrides do not reuse stale agent models. Anthropic also accepts `ANTHROPIC_API_KEY` plus optional `ANTHROPIC_BASE_URL`; custom Anthropic HTTPS origins require `BEATER_ANTHROPIC_ALLOW_CUSTOM_BASE_URL=1`, and insecure Anthropic HTTP is accepted only for loopback fixtures with `BEATER_ANTHROPIC_ALLOW_INSECURE_LOOPBACK=1`. OpenAI-compatible providers also accept `BEATER_OPENAI_API_KEY` or `OPENAI_API_KEY` plus `BEATER_OPENAI_BASE_URL` or `OPENAI_BASE_URL`; custom HTTPS origins require `BEATER_OPENAI_ALLOW_CUSTOM_BASE_URL=1`, and insecure HTTP is accepted only for loopback fixtures with `BEATER_OPENAI_ALLOW_INSECURE_LOOPBACK=1`. NVIDIA should be selected with `BEATER_LLM_PROVIDER=nvidia`, `BEATER_LLM_MODEL`, and `BEATER_NVIDIA_API_KEY` or `NVIDIA_API_KEY`; `BEATER_NVIDIA_BASE_URL` or `NVIDIA_BASE_URL` can override the pinned default endpoint, custom HTTPS origins require `BEATER_NVIDIA_ALLOW_CUSTOM_BASE_URL=1` or `BEATER_OPENAI_ALLOW_CUSTOM_BASE_URL=1`, and insecure HTTP is accepted only for loopback fixtures with `BEATER_NVIDIA_ALLOW_INSECURE_LOOPBACK=1` or `BEATER_OPENAI_ALLOW_INSECURE_LOOPBACK=1`. Secret-bearing LLM clients disable redirects and proxy routing, and provider error bodies are omitted from agent errors so a misconfigured endpoint cannot echo secrets into the terminal or journal.
