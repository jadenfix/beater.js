...
| 7 | **LLM streaming** — SSE to browser + partial-step journal records | **partial:** Anthropic SSE ingestion, partial-step journal records, and a browser-consumable run events stream are implemented; done requires polished page/UI integration for live runs |
| 8 | **MCP consume + sessions** — use remote MCP servers as tool sources; add session/auth plumbing for remote management; adopt the next MCP spec when released | **partial:** declared remote MCP tools support scoped credentials, egress, retries, idempotency keys, review parking, and provider session initialization; done requires provider `tools/list` discovery/schema import and next-spec transport adoption |
