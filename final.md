...
| 12 | **Observability** — OTLP out of the agent loop into beater-agents | **partial:** opt-in native and OTLP/HTTP exporters post finished journal runs and steps to `/v1/traces/native` or `/v1/traces`; `scripts/otlp-trace-gate.cjs` proves the local OTLP collector path, and `scripts/beater-dashboard-trace-gate.cjs` proves a real Beater native ingest/read flow through the endpoints the dashboard uses. Done still requires rendered beater-agents dashboard proof for an exported run trace |
