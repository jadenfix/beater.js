...
| 4 | **npm/node-compat** — the adoption wedge (Deno-style compat layer, not a reimplementation) | **done for the wedge:** `import { z } from "zod"` works in a route, leaf `.cjs` packages can be imported as default exports, and resolver tests cover exact/wildcard `exports`, array export targets, server-side conditions, package-boundary rejection including symlinks, `module`/`main` fallbacks, app-local `import_map.json` exact/prefix aliases, `.cjs` wrapping, and client `.cjs` rejection; CommonJS `require` fails closed, while Node built-ins, install hooks, and client bundling remain later work |
